...
Vagrant.configure("2") do |config|
  ...
  (0..2).each do |i|
    config.vm.define "k8s-#{i}" do |node|
      ...
      SHELL
      if i == 0 
        node.vm.provision "shell", inline: "kubeadm init --token #{token} --apiserver-advertise-address 192.168.50.4 --pod-network-cidr 10.244.0.0/16"
        ...
        node.vm.provision "shell", inline: "kubeadm join 192.168.50.4:6443 --token #{token} --discovery-token-unsafe-skip-ca-verification"
        node.vm.provision "shell", inline: "mkdir /home/vagrant/.kube"
